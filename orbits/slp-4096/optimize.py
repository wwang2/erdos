...
n=4096, seeded from slp-1024-multistart solution via tiling (repeat each element 4x).

Strategy:
1. Seed from slp-1024-multistart solution tiled to 4096 steps (np.repeat x4)
2. Iteratively solve LP subproblems to minimize the max cross-correlation
3. Active-set selection: top-K shifts near the maximum (capped at 300)
4. Sparse line search (20 alphas) for step size
5. Save checkpoints every 25 iterations
...
SEED_PATH = _MAIN_REPO / "orbits/slp-1024-multistart/solution.py"
...
# Cap active shifts to keep LP tractable at n=4096
MAX_ACTIVE = 300


def load_seed_tiled():
    """Load slp-1024-multistart solution and tile to 4096 by repeating each element 4x."""
    ns = {}
    exec(SEED_PATH.read_text(), ns)
    h1024 = np.asarray(ns["h_values"], dtype=float)
    h4096 = np.repeat(h1024, 4)
    # Rescale to ensure exact sum
    h4096 *= N_HALF / h4096.sum()
    h4096 = np.clip(h4096, 0.0, 1.0)
    return h4096
...
def line_search(h, h_lp, current_obj, n_steps=20):
    """Sparse line search (20 alphas) to keep per-iteration cost low at n=4096."""
    best_alpha = 0.0
    ...
    if best_alpha > 0:
        for alpha in np.linspace(best_alpha * 0.5, best_alpha * 2.0, 10):
            h_new = project_to_feasible(h + alpha * (h_lp - h))
            obj, _ = compute_upper_bound(h_new)
            if obj < best_obj:
                best_obj = obj
...
def run_slp(n_iter=150, checkpoint_every=25, seed_h=None,
            init_delta=0.05, init_threshold=1e-3, no_improve_limit=30):
    if seed_h is not None:
        h = project_to_feasible(seed_h.copy())
        print(f"  Starting from seed: obj={compute_upper_bound(h)[0]:.15f}")
    else:
        print(f"Loading slp-1024-multistart tiled to {N} steps...")
        h = load_seed_tiled()
    ...
    for it in range(1, n_iter + 1):
        ...
        if it % 5 == 0 or it <= 5:
            ...
            print(f"  Iter {it:5d}: obj={new_obj:.15f}  alpha={alpha:.6f}  "
                  f"active={n_active}  delta={delta:.4f}  improve={improvement:.2e}  "
                  f"iter_t={iter_elapsed:.1f}s  total_t={elapsed:.1f}s")
            sys.stdout.flush()
...
def run_with_restarts(n_restarts=8, n_iter_per_run=150):
    print("=== Initial run from tiled slp-1024-multistart seed ===")
    best_h, best_obj = run_slp(n_iter=n_iter_per_run, checkpoint_every=25,
                                init_delta=0.05, no_improve_limit=30)
    save_solution(best_h)
    np.save(CHECKPOINT_PATH, best_h)
    print(f"Initial best: {best_obj:.15f}")

    print(f"\n--- Perturbation restarts (best: {best_obj:.15f}) ---")
    # Small perturbation scales — lessons from slp-1024-multistart: 0.01-0.02 is ideal
    scales = [0.005, 0.01, 0.02, 0.005, 0.01, 0.02, 0.005, 0.01]

    for restart in range(n_restarts):
        scale = scales[restart % len(scales)]
        np.random.seed(restart + 700)
        ...
        h_new, obj_new = run_slp(n_iter=n_iter_per_run, checkpoint_every=25,
                                  seed_h=h_perturbed, init_delta=0.05, no_improve_limit=30)
...
def main():
    best_h, best_obj = run_with_restarts(n_restarts=8, n_iter_per_run=150)
    save_solution(best_h)
    np.save(CHECKPOINT_PATH, best_h)

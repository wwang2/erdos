...
Strategy:
1. Seed from Together AI 512-step solution (interpolated to 1024 steps)
...
BASELINE_PATH = REPO_ROOT / "research/eval/baselines/together_ai_2026.py"
...
def run_slp(n_iter=5000, top_k=10, checkpoint_every=100, seed_h=None):
    ...
    if seed_h is not None:
        ...
        print(f"Loading baseline and interpolating to {N} steps...")
        h_base = load_baseline()
        print(f"  Baseline: shape={h_base.shape}, sum={h_base.sum():.4f}")

        h = interpolate_to_n(h_base, N)
        h = project_to_feasible(h)
        print(f"  Interpolated: shape={h.shape}, sum={h.sum():.6f}")
...
def run_with_restarts(n_restarts=20, n_iter_per_run=500, top_k=10):
    """Run SLP with perturbation restarts to escape local minima."""
    # Initial run from interpolated baseline
    print("=== Initial run ===")
    ...
    for restart in range(n_restarts):
        # Cycle through different perturbation scales
        scales = [0.05, 0.02, 0.01, 0.005, 0.1, 0.003, 0.001, 0.2, 0.15, 0.08]
...
def main():
    best_h, best_obj = run_with_restarts(n_restarts=100, n_iter_per_run=300, top_k=10)
    save_solution(best_h)
    np.save(CHECKPOINT_PATH, best_h)
